...
class Auth extends CI_Controller 
{
  ...
  public function on_login()
  {
    ...
    if ($req->status && $req->status_code == 200)
    {
      ...
      $this->_setAuthSession($data, $token);
    }
  ...
  public function session_expired()
  {
    $data = [
      ...
      'script' => [
        base_url('assets/js/auth/SessionExpired.js' . versionAssets())
      ]
    ];
  ...
  public function on_lockScreen()
  {
    ...
    $send = [
      'username'     => decode(getReqBody('username', '')),
      'password'     => getReqBody('password', ''),
      'clientId'     => env('CLIENT_ID'),
      'clientSecret' => env('CLIENT_SECRET')
    ];
  ...
  private function _setAuthSession($data, $token)
  {
    $session_log = [
      'log_userId'        => $data->id,
